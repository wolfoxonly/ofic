...
// Copyright (c) 2011-2015 The OFIChain developers
...
const string strMessageMagic = "OFIChain Signed Message:\n";
...
void ThreadScriptCheck() {
    RenameThread("OFIChain-scriptch");
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    // ppcoin: fees are not collected by miners as in OFIChain
...
bool LoadBlockIndex()
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsLimit =%d,nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "OFIChain", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnProofOfWorkLimit.GetCompact(),bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
...
// ~uint256(0) >> 32
    ...
    if (!fReindex) {
        ...
        CTransaction txNew;
        txNew.nTime = 1518513782;//1516498115;//1345083810;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1518513782;// 1345084287;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
      //  block.nNonce   = 2179302059u;
        block.nNonce = ByteReverse(25931990);//<zxb>
        if (fTestNet)
        {
...
/////////////////////////////////zxb11111111111111111111111
 ...
 printf("block.nNonce 11111111111111 zxb:%d \n" ,block.nNonce);
        ...
        assert(block.hashMerkleRoot == uint256("0xc71a49fcd30e8cba1de841e3c1e6558e6f4a71cf89b518187bb9209817e93caa"));
    ...
    int64 nFees = 0;
    {
        ...
        if (pblock->IsProofOfWork())
        {
          ...
          printf("POW pindexPrev.nHeight===%d\n", pindexPrev->nHeight);
           ...
           {
                pblock->vtx[0].vout[0].nValue = 180000000 * COIN;//首块产量<zxb>
           }
           else
           {
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("OFIChainMiner : proof-of-work not meeting target");

    //// debug print
    printf("OFIChainMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OFIChainMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OFIChainMiner : ProcessBlock, block not accepted");
    }
...
#ifdef TESTING
void OFIChainMiner(CWallet *pwallet, bool fProofOfStake, bool fGenerateSingleBlock)
#else
void OFIChainMiner(CWallet *pwallet, bool fProofOfStake)
#endif
{
    ...
    try { loop {
        ...
        printf("Running OFIChainMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        if(!fProofOfStake) printf("OFIChainMiner terminated\n");
        throw;
    }
}

void GenerateOFIChains(bool fGenerate, CWallet* pwallet)
{
...
#ifdef TESTING
        minerThreads->create_thread(boost::bind(&OFIChainMiner, pwallet, false, false));
#else
        minerThreads->create_thread(boost::bind(&OFIChainMiner, pwallet, false));
#endif
}
