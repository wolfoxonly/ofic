...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\OFIChain
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\OFIChain
    // Mac: ~/Library/Application Support/OFIChain
    // Unix: ~/.OFIChain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "OFIChain";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "OFIChain.conf"));
    ...
    if (mapArgs.count("-conf") == 0 && !boost::filesystem::exists(pathConfigFile)) {
        ...
        else
        {
            ...
            try {
                const char* content = "rpcuser=OFIChainrpc\nrpcpassword=OFIChainrpcpassword";
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No OFIChain.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override OFIChain.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "OFIChaind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong OFIChain will not work properly.");
...
// Format the subversion field according to BIP 14 spec (https://en.OFIChain.it/wiki/BIP_0014)
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments)
{
    ...
    ss << "/";
    ss << "OFIChain:" << FormatVersion(OFICHAIN_VERSION);
    ss << "(" << CLIENT_BUILD << ")/";
    return ss.str();
}
