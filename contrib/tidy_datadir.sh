#!/bin/bash
# Copyright (c) 2013 The OFIChain Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete OFIChain database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no OFIChain datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old OFIChain datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected OFIChain 0.7 datadir."
    ;;
  3)
    echo "Detected OFIChain pre-0.8 datadir."
    ;;
  4)
    echo "Detected OFIChain 0.8 datadir."
    ;;
esac
