...
The RPM spec file provided here is for OFIChain-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://OFIChain.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[OFIChain git repository](https://github.com/OFIChain/OFIChain) but are not part of
...
Currently only `Source10` is used, for the example `OFIChain.conf` file.
...
in the OFIChain git repository. At present only an SVG version of the OFIChain
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the OFIChain
...
needed when 0.12.1 is released, a proper fix is already in the OFIChain git
master branch.
...
idea to build OFIChain Core against a static version of that library compiled
...
The `OFIChain.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `OFIChain-server` package. At this point, most RPM based
...
The `OFIChain-server` package should have SELinux support. How to properly do
...
OFIChain-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/OFIChain/). He has been
packaging OFIChain for Fedora at least since 2012.
...
4. I renamed the `OFIChain` package that contains the Qt GUI to `OFIChain-core` as
that appears to be how the general population refers to it, in contrast to
`OFIChain-xt` or `OFIChain-classic`. I wanted to make sure the general population
