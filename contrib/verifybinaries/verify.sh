...
###   OFIChaincore.org and OFIChain.org and compares them.
...
WORKINGDIR="/tmp/OFIChain_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://OFIChaincore.org"
HOST2="https://OFIChain.org"
BASEDIR="/bin/"
VERSIONPREFIX="OFIChain-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'OFIChain-',
...
if [ $? -ne 0 ]; then
   echo "OFIChain.org failed to provide signature file, but OFIChaincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "OFIChain.org and OFIChaincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the OFIChain Core binary release signing key installed?"
   fi
